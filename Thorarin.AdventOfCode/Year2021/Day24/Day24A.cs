...
public class Day24A : Day24Base
{
    ...
    //                 if (nw != null)
    //                 {
    //                     newStates.TryAdd(nw.RelevantState, nw);
    //                 }
    //             }
    //         }
    //
    ...
    //     }
    //
    //     return states.Where(x => x.RelevantState == 0).Select(x => x.Number).Max();
    // }    
    
    long? Find3(IReadOnlyList<State> states)
    {
        for (int i = 0; i < 14; i++)
        {
            Console.WriteLine($"Processing {states.Count:00} states");
            Dictionary<long, State> newStates = new Dictionary<long, State>();
